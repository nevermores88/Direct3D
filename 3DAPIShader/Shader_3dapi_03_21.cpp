...
CShader_3dapi_03_21::CShader_3dapi_03_21()
{
}
...
HRESULT CShader_3dapi_03_21::Create(LPDIRECT3DDEVICE9 pdev)
{
	CBaseClass::Create(pdev);

	m_pShader = LoadShader("Ex03_21/Shader.fx");
	if (!m_pShader)
		return E_FAIL;
	...
	D3DXCreateTextureFromFile(m_pdev, "Ex03_21/Toon.png", &m_pTex);

	m_pMesh = NULL;
	m_pMesh = new CMesh;
	m_pMesh->Create(m_pdev, "Ex03_21/teapot.x");

	return S_OK;
}

void CShader_3dapi_03_21::Release()
{
	if (m_pShader)
	{
		m_pShader->Release();
...
void CShader_3dapi_03_21::Render()
{
	if (m_pdev)
	{
		D3DXMATRIX mtView;
		...
		D3DXMATRIX mtProj;
		D3DXCOLOR	toonColor(0.1f, 0.8f, 1.0f, 1);
		...
		m_pShader->SetVector("g_vToonColor", (D3DXVECTOR4*)&toonColor);
...
void CShader_3dapi_03_21::Update()
{
	if (m_pdev)
	{
		D3DXMATRIX mtScale;
