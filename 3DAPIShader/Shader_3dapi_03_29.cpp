...
CShader_3dapi_03_29::CShader_3dapi_03_29()
{
}
...
HRESULT CShader_3dapi_03_29::Create(LPDIRECT3DDEVICE9 pdev)
{
	CBaseClass::Create(pdev);

	m_pShader = LoadShader("Ex03_29/Shader.fx");
	if (!m_pShader)
		return E_FAIL;

	D3DXCreateTextureFromFile(m_pdev, "Ex03_29/rock_n.tga", &m_pNormalTex);
	...
	for (DWORD ring = 0; ring < iNSphereSegments; ring++)
	{
		...
		for (DWORD seg = 0; seg < (iNSphereSegments + 1); seg++)
		{
			...
			D3DXVec3Cross(&pVertices->b, &pVertices->t, &pVertices->n);
			D3DXVec3Normalize(&pVertices->b, &pVertices->b);
...
void CShader_3dapi_03_29::Release()
{
	if (m_pShader)
	{
		m_pShader->Release();
...
void CShader_3dapi_03_29::Render()
{
	if (m_pdev)
	{
		D3DXMATRIX mtWorld;
...
void CShader_3dapi_03_29::Update()
{
	if (m_pdev)
	{
		D3DXMATRIX	mtTM;
