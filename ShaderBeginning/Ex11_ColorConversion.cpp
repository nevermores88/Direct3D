...
CEx11_ColorConversion::CEx11_ColorConversion()
{
}
...
HRESULT CEx11_ColorConversion::Create(LPDIRECT3DDEVICE9 pdev)
{
	CBaseClass::Create(pdev);

	m_pEnvironmentMappingShader = LoadShader("Ex11/EnvrionmentMapping.fx");
	if (!m_pEnvironmentMappingShader)
		return E_FAIL;

	m_pNoEffect = LoadShader("Ex11/NoEffect.fx");
	if (!m_pNoEffect)
		return E_FAIL;

	m_pGrayScale = LoadShader("Ex11/Grayscale.fx");
	if (!m_pGrayScale)
		return E_FAIL;

	m_pSepia = LoadShader("Ex11/Sepia.fx");
	if (!m_pSepia)
		return E_FAIL;

	m_pTeapot = LoadModel("Ex11/TeapotWithTangent.x");
	if (!m_pTeapot)
		return E_FAIL;

	m_pTexDM = LoadTexture("Ex11/Fieldstone_DM.tga");
	if (!m_pTexDM)
		return E_FAIL;

	m_pTexSM = LoadTexture("Ex11/Fieldstone_SM.tga");
	if (!m_pTexSM)
		return E_FAIL;

	m_pTexNM = LoadTexture("Ex11/Fieldstone_NM.tga");
	if (!m_pTexNM)
		return E_FAIL;

	D3DXCreateCubeTextureFromFile(m_pdev, "Ex11/Snow_ENV.dds", &m_pSnowENV);
	if (!m_pSnowENV)
		return E_FAIL;
...
void CEx11_ColorConversion::Release()
{
	if (m_pEnvironmentMappingShader)
	{
...
void CEx11_ColorConversion::Render()
{
	if (m_pdev)
	{
...
void CEx11_ColorConversion::Update()
{
	if (m_pdev)
	{
...
void CEx11_ColorConversion::InitFullScreenQuad()
{
