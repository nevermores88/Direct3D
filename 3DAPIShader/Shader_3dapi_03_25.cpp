...
CShader_3dapi_03_25::CShader_3dapi_03_25()
{
}
...
HRESULT CShader_3dapi_03_25::Create(LPDIRECT3DDEVICE9 pdev)
{
	CBaseClass::Create(pdev);

	m_pShader = LoadShader("Ex03_25/Shader.fx");
	if (!m_pShader)
		return E_FAIL;
	...
	{
		char szPath[128];
		sprintf(szPath, "Ex03_25/stroke%d.bmp", i);

		D3DXCreateTextureFromFile(m_pdev, szPath, &m_pTex[i]);
	}

	D3DXCreateTextureFromFile(m_pdev, "Ex03_25/wood.jpg", &m_pDiffuseTex);

	LPD3DXMESH	pOrgMesh = NULL;
	LPD3DXBUFFER pAdj = NULL;

	D3DXLoadMeshFromX("Ex03_25/teapot.x", D3DXMESH_SYSTEMMEM, m_pdev, &pAdj, NULL, NULL, NULL, &pOrgMesh);
...
void CShader_3dapi_03_25::Release()
{
	if (m_pShader)
	{
		m_pShader->Release();
...
void CShader_3dapi_03_25::Render()
{
	if (m_pdev)
	{
...
void CShader_3dapi_03_25::Update()
{
	if (m_pdev)
	{
		D3DXMATRIX mtScale;
